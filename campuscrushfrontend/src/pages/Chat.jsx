...
const Chat = () => {
    ...
    const handleReveal = async () => {
        ...
        const handleBlock = async () => {
            if (!window.confirm("Are you sure you want to block this conversation? This cannot be undone.")) return;
            try {
                await api.post(`/confessions/${confessionId}/block`);
                setConfessionState('BLOCKED');
            ...
            <div className="container" style={{ height: '100vh', display: 'flex', flexDirection: 'column' }}>
                <header style={{ padding: '1rem', borderBottom: '1px solid #eee', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    ...
                    <div style={{ display: 'flex', gap: '0.5rem' }}>
                        ...
                        {isSender && !isRevealed && confessionState !== 'CREATED' && confessionState !== 'BLOCKED' && (
                            <Button
                                variant="secondary"
                        ...
                        {/* Block Button for Receiver */}
                        {!isSender && confessionState !== 'BLOCKED' && (
                            <Button
                                variant="danger"
                                onClick={handleBlock}
                                style={{ fontSize: '0.8rem', padding: '0.25rem 0.75rem', backgroundColor: '#fee2e2', color: '#dc2626', border: '1px solid #fecaca' }}
                            >
                                Block 🚫
                            </Button>
                        )}
                ...
                <div style={{ flex: 1, overflowY: 'auto', padding: '1rem', display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                    ...
                    {confessionState === 'BLOCKED' && !loading && (
                        <div style={{ textAlign: 'center', color: '#ef4444', margin: '1rem 0', padding: '1rem', background: '#fef2f2', borderRadius: '8px', border: '1px solid #fecaca' }}>
                ...
                <form onSubmit={handleSend} style={{ padding: '1rem', borderTop: '1px solid #eee', backgroundColor: 'white', display: 'flex', gap: '0.5rem' }}>
                    <input
                        ...
                        placeholder={confessionState === 'CREATED' ? "Waiting for reply..." : confessionState === 'BLOCKED' ? "Conversation blocked" : "Type a message..."}
                        disabled={confessionState === 'CREATED' || confessionState === 'BLOCKED'}
                    />
                    <Button type="submit" disabled={confessionState === 'CREATED' || confessionState === 'BLOCKED'}>Send</Button>
                </form>
            </div>
        );
    };
